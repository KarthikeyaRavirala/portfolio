...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index }) => {
  ...
  return (
    <a 
      ...
      className={`block relative ${getBackgroundColor(index)} border border-white/10 rounded-xl p-6 transition-all duration-300 hover:border-white/30 group hover:shadow-[0_0_20px_rgba(59,130,246,0.5)]`}
    >
      <h3 className="text-2xl font-bold text-white" style={{ fontFamily: "'Josefin Sans', sans-serif" }}>
        {getTitleWithEmoji(project.title)}
      </h3>
      <p className="text-gray-300 mt-4 font-normal" style={{ fontFamily: "'Josefin Sans', sans-serif" }}>{project.description}</p>
      <p className="text-base text-gray-500 mt-6 font-normal" style={{ fontFamily: "'Josefin Sans', sans-serif" }}>{project.stats}</p>
